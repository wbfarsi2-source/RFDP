...
WAIT_MESSAGE = "منتظر بمانید و تا انجام بروزرسانی، درخواست دوباره ارسال نکنید."
...
def _format_result(report: dict[str, Any]) -> str:
    ...
    if isinstance(top3, list) and top3:
        ...
        lines.append("نتیجه‌ای شناسایی نشد.")
    unidentified = _unidentified_names(report)
    if unidentified:
        lines.extend(["", "شناسایی نشد: " + "، ".join(unidentified)])
    return "\n".join(lines)
...
async def _run_scan(bot: Any, progress_message: Message, requester_id: int) -> None:
    try:
        ...
        await _safe_edit(progress_message, "بروزرسانی انجام نشد. کمی بعد دوباره امتحان کنید.")
...
async def _accept_request(bot: Any, chat_message: Message, requester_id: int) -> None:
    ...
    async with _lock():
        ...
        if cooldown_until > now:
            ...
            await chat_message.answer(f"بروزرسانی بعدی حدود {minutes} دقیقه دیگر فعال می‌شود.")
            return
        _write_state({
            "running": True,
            "accepted_at": now,
