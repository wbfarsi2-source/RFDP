...
def _format_result(report: dict[str, Any]) -> str:
    ...
    lines.extend([
        "",
        f"بررسی‌شده: {checked}/25 | شناسایی‌شده: {successful} | شناسایی‌نشده: {len(unidentified)}",
        "درخواست بعدی ۶ دقیقه پس از پایان این بررسی پذیرفته می‌شود.",
    ])
    return "\n".join(lines)
...
async def _run_scan(bot: Any, progress_message: Message, requester_id: int) -> None:
    ...
    try:
        ...
        await _safe_edit(
            ...
            "تا ۶ دقیقه درخواست تازه پذیرفته نمی‌شود.\n\n"
...
async def _accept_request(bot: Any, chat_message: Message, requester_id: int) -> None:
    ...
    progress_message = await chat_message.answer(
        ...
        "شروع بررسی هر سرور با فاصلهٔ ۶ تا ۸ ثانیه از سرور قبلی تنظیم می‌شود."
    )
