...
SERVER_START_GAP_MIN_SECONDS = 6.0
SERVER_START_GAP_MAX_SECONDS = 8.0
FAILED_SERVER_RETRY_LIMIT = 1
...
def _proxy_openers(root: Path) -> list[urllib.request.OpenerDirector]:
    ...
    # AUTO is direct-first. An explicit proxy keeps proxy-first behavior but
    # still has direct as a safe fallback. DIRECT/NONE/OFF never uses a proxy.
    if mode in {"direct", "none", "off"}:
        return [direct]

    openers: list[urllib.request.OpenerDirector] = []
    if mode == "auto":
        openers.append(direct)
...
def _ws_proxy_attempts(root: Path) -> list[dict[str, Any]]:
    ...
    candidates: list[str] = []
    if mode == "auto":
        # Direct first; system proxy is only a fallback.
        attempts: list[dict[str, Any]] = [direct]
        proxies = urllib.request.getproxies()
...
def _retryable_failure(result: ServerResult) -> bool:
    """Allow one conservative retry for unavailable/transient server checks.

    This does not bypass membership or access restrictions. It only refreshes
    authentication/server metadata and repeats the same normal connection flow.
    """
    if result.ok:
        return False
    ...
    markers = (
        ...
        "queue failed",
        "queue evicted",
        "no verified ember player snapshot",
    )
    return any(marker in text for marker in markers)


def _refresh_server(root: Path, cookie: str, number: int, original: dict[str, Any]) -> dict[str, Any]:
    """Refresh the server route metadata before a retry."""
    try:
        auth_status, _, _ = _http_get(root, "/api/auth/me", cookie, timeout=12.0)
        if auth_status != 200:
            return original
        status, payload, _ = _http_get(root, "/api/servers", cookie, timeout=15.0)
        if status != 200 or not isinstance(payload, dict):
            return original
        for row in payload.get("servers") or []:
            if isinstance(row, dict) and _server_number(row) == number:
                return dict(row)
    except Exception:
        pass
    return original
...
def _scan_one_with_retry(
    root: Path,
    server: dict[str, Any],
    cookie: str,
    own_id: int,
) -> ServerResult:
    first = _scan_one(root, server, cookie, own_id)
    if first.ok or FAILED_SERVER_RETRY_LIMIT <= 0 or not _retryable_failure(first):
        return first

    # Keep the retry separated from the failed connection attempt as well.
    retry_wait = random.SystemRandom().uniform(
        SERVER_START_GAP_MIN_SECONDS,
        SERVER_START_GAP_MAX_SECONDS,
    )
    time.sleep(retry_wait)
    refreshed = _refresh_server(root, cookie, first.number, server)
    second = _scan_one(root, refreshed, cookie, own_id)
    second.attempts = 2
    if not second.ok:
        second.error = (
            f"attempt 1: {first.error}; attempt 2: {second.error}"
        )[:500]
    return second
...
def scan_all_servers(progress: Callable[[int, int, ServerResult], None] | None = None) -> dict[str, Any]:
    ...
    for index, server in enumerate(servers, start=1):
        ...
        result = _scan_one_with_retry(root, server, cookie, own_id)
        results.append(result)
        if progress is not None:
            progress(index, len(servers), result)
        # Spread consecutive server starts over a 6-to-8-second window.
        target_gap = rng.uniform(
            SERVER_START_GAP_MIN_SECONDS,
            SERVER_START_GAP_MAX_SECONDS,
        )
        selected_gaps.append(target_gap)
        next_start = actual_start + target_gap

    successful = [row for row in results if row.ok and row.count is not None]
    unidentified = [row for row in results if not row.ok or row.count is None]
    ...
    report = {
        ...
        "servers_failed": len(unidentified),
        "servers_unidentified": len(unidentified),
        "unidentified_servers": [
            {"server": row.server, "number": row.number, "attempts": row.attempts}
            for row in unidentified
        ],
        "minimum_server_start_gap_seconds": SERVER_START_GAP_MIN_SECONDS,
        "maximum_server_start_gap_seconds": SERVER_START_GAP_MAX_SECONDS,
        "selected_start_gaps_seconds": [round(value, 3) for value in selected_gaps],
        "retry_limit_per_server": FAILED_SERVER_RETRY_LIMIT,
